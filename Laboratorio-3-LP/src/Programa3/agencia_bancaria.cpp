...
bool Agencia_Bancaria::criar_conta(){
	...
	while (true)
	{
		if (select == 'p')
		{
			nova_conta = std::make_shared<Poupanca>( agencia, numero, saldo );
			break;
		}
		else if (select == 'c')
		{
			...
			nova_conta = std::make_shared<Corrente>( agencia, numero, saldo, status );
			break;
		}

		std::cout << std::endl << ">>> A conta a ser criada é poupança ou corrente? ( p / c ): ";
		select = get_one_char();
	}
...
bool Agencia_Bancaria::alterar_status(){
	...
	while (true)
	{
		...
		std::cout << std::endl << ">>> Qual é o novo status? Especial ou não? ( s / n ): ";
